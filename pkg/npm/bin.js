...
const binName = isWindows ? 'codebase-memory-cli.exe' : 'codebase-memory-cli';
...
if (!cacheReady()) {
  ...
  process.stderr.write('codebase-memory-cli: binary not found, downloading...\n');
  const installResult = spawnSync(process.execPath, [path.join(__dirname, 'install.js')], {
    stdio: 'inherit',
  });
  if (installResult.status !== 0 || !cacheReady()) {
    process.stderr.write(
      'codebase-memory-cli: download failed.\n' +
      'Try reinstalling: npm install -g codebase-memory-cli\n'
    );
    process.exit(1);
  }
}
...
if (result.error) {
  process.stderr.write(`codebase-memory-cli: ${result.error.message}\n`);
  process.exit(1);
}
...
if (mutation) {
  ...
  process.stderr.write(
    ...
    '"codebase-memory-cli install --yes" once for a managed install with its ' +
